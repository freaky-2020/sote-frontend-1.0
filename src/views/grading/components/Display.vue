...
<!--      </div>-->
      ...
      <div  v-for="(question,index) in exam_date[5]" :key="index+'5'">
        <div v-if="num === (index+exam_date[1].length+exam_date[2].length+exam_date[3].length+exam_date[4].length)">
          <div slot="header" class="clearfix">
            <h3 class="box-center">简答题(共{{ exam_date[5].length }}题，合计{{ getAllScore(exam_date[5]) }}分)</h3>
          </div>
          <h3 class="box-center">{{ index+1 }}、{{ replace_stem(question.stem) }}</h3>
          <el-input v-model="detailDate[question.quesNo-1].answer" type="textarea" :rows="5" resize="none" maxlength="500" style="margin-bottom: 5px" />
